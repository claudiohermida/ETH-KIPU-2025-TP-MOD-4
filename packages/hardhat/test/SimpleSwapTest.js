...
describe("SimpleSwap", function () {
  ...
  describe("Liquidity Provision", function () {
    ...
    it("addLiquidityshould revert if deadline expired", async function () {
      ...
      await expect(
        simpleSwap.connect(deployer).addLiquidity(
          tokenA.target,
          tokenB.target,
          amountA,
          amountB,
    ...
    it("should revert if target address is invalid", async function () {
      ...
      await expect(
        simpleSwap
          .connect(deployer)
          .addLiquidity(tokenA.target, tokenB.target, amountA, amountB, amountA, amountB, ZERO_ADDRESS, newTimestamp),
    ...
    it("should not add liquidity if zero liquidity would be minted", async function () {
      ...
      await expect(
        simpleSwap.addLiquidity(tokenA.target, tokenB.target, 0n, 0n, 0n, 0n, deployer.address, newTimestamp + 60),
  ...
  describe("Liquidity Removal", function () {
    ...
    it("should revert if deadline expired", async function () {
      ...
      await expect(
        simpleSwap.connect(deployer).removeLiquidity(
          tokenA.target,
          tokenB.target,
          initialLiquidity,
          initialASupply,
  ...
  describe("Token Swap", function () {
    ...
    it("should revert if deadline expired", async function () {
      ...
      // Attempt to remove liquidity with a past deadline
      await expect(
        simpleSwap.connect(deployer).swapExactTokensForTokens(
          amountIn,
          amountOutMin,
    ...
    it("should revert if target address is invalid", async function () {
      ...
      await expect(
        simpleSwap
          .connect(deployer)
          .swapExactTokensForTokens(amountIn, amountOutMin, [tokenA.target, tokenB.target], ZERO_ADDRESS, newTimestamp),
      ).to.be.revertedWithCustomError(simpleSwap, "INVALID_TO");
    });
    ...
    it("should revert if the tokens provided are not A and B and distinct", async function () {
      ...
      await expect(
        simpleSwap
          .connect(deployer)
          .swapExactTokensForTokens(initialASupply, 0n, [tokenA.target, tokenB.target], deployer.address, newTimestamp),
      ).not.to.be.reverted;

      await expect(
        simpleSwap
          .connect(deployer)
          .swapExactTokensForTokens(initialASupply, 0n, [tokenB.target, tokenA.target], deployer.address, newTimestamp),
      ).not.to.be.reverted;
    });
    ...
    it("should swap tokens, emit event and transfer tokens", async function () {
      ...
      await expect(
        ...
        .withArgs(deployer.address, deployer.address, tokenA.target, tokenB.target, amountIn, amountOut);
  ...
  describe("getAmountOut", function () {
    ...
    it("getAmountOut should revert if reserve and amountIn are zero", async function () {
      ...
      await expect(simpleSwap.getAmountOut(0n, 0n, 100n)).to.be.reverted;
    });
  });
  ...
  describe("getPrice", function () {
    ...
    it("getPrice should revert if there is no liquidity", async function () {
      ...
      await expect(simpleSwap.getPrice(tokenA.target, tokenB.target)).to.be.revertedWithCustomError(
        simpleSwap,
